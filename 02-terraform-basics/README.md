...
Если вставить здесь ключ, то получиться хардкод, поэтому добавим ключ через локальную переменную в `locals`
...
В файле `main.tf` будем использовать в ресурсах локальную переменную
